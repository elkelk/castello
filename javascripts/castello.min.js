...
var Constants,Tile,TileTypes;Constants={features:{field:"field",road:"road",city:"city",cloister:"cloister",stop:"stop"}};TileTypes={};TileTypes.get=function(features){return _.map(features,function(feature){return Constants.features[feature];});};TileTypes.starter=TileTypes.get(["stop","city","stop","road","field","field","field","road","road"]);TileTypes.diagonal_city=TileTypes.get(["stop","city","city","city","stop","field","field","field","stop"]);TileTypes.straight_city=TileTypes.get(["stop","city","stop","field","stop","city","stop","field","city"]);TileTypes.half_city=TileTypes.get(["stop","city","stop","field","field","field","field","field","field"]);TileTypes.double_half_city=TileTypes.get(["stop","city","stop","field","stop","city","stop","field","field"]);TileTypes.three_sided_city=TileTypes.get(["city","city","city","city","stop","field","stop","city","city"]);TileTypes.elbow_road=TileTypes.get(["field","road","field","road","field","field","field","field","road"]);TileTypes.straight_road=TileTypes.get(["field","road","field","field","field","road","field","field","road"]);TileTypes.t_road=TileTypes.get(["field","road","field","road","field","road","field","field","stop"]);TileTypes.t_road_ctiy=TileTypes.get(["stop","road","field","road","field","road","stop","city","stop"]);Tile=Backbone.Model.extend({defaults:{features:TileTypes.starter,position_x:0,position_y:0,placed:false,canvas:"#main_canvas"},initialize:function(){},place:function(x,y){return this.set({position_x:x,position_y:y});},place_player:function(color,index){return this.set({player_on:index,player_color:color});},feature:function(index){return this.get("features")[index];},draw:function(){this.draw_tile();return this.draw_city();},draw_tile:function(){return $(this.canvas).drawRect({fillStyle:"#000",x:this.position_x,y:this.position_y,width:100,height:100,fromCenter:false});},draw_city_part:function(side){var end,start,x,y,_ref;_ref=(function(){switch(side){case 1:return[-Math.PI/2,Math.PI/2,50,0];case 3:return[0,Math.PI,100,50];case 5:return[Math.PI/2,-Math.PI/2,50,100];case 7:return[Math.PI,0,0,50];}})(),start=_ref[0],end=_ref[1],x=_ref[2],y=_ref[3];return $("#main_canvas").drawArc({fillStyle:"brown",x:x+this.position_x,y:y+this.position_y,radius:50,start:start,end:end,ccw:true,inDegrees:false});},draw_city:function(){return _.each([1,3,5,7],function(side){if(this.feature(side)===Constants.features.city){return draw_city_part(side);}});},feature_continues:function(index,direction){var adjacent_city,adjacent_field,center_same,feature,feature_continues,neighboring_field,opposite,opposite_neighboring_field;if(direction==null){direction=null;}
...
return index;}});(function(){var tile;tile=new Tile({features:TileTypes.starter});return tile.draw();})();