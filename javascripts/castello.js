...
Tile = Backbone.Model.extend({
  ...
  feature_end: function(feature_index) {
    var feature, feature_end, side, that;
    that = this;
    feature_end = true;
    feature = this.get("features")[feature_index];
    side = this.feature_side(feature_index);
    if (feature === Constants.features.city) {
      if (side === "north") {
        _.each(_.union(Tile.east(), Tile.west()), function(test_index) {
          if (feature === that.get("features")[test_index]) {
            return feature_end = false;
          }
        });
      }
    }
